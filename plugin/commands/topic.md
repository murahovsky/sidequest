...
4. If it printed a line starting with `ok`: run `sh "${CLAUDE_PLUGIN_ROOT}/scripts/run.sh" generate-rest` (returns instantly; a background job tops the pool up). Then reply with ONE calm sentence in the user's language: everything is set — facts about <topic> are in the spinner; `/sidequest:topic <new topic>` switches, `/sidequest:off` turns off. STRICT: no numbers, no "pool"/"background"/generation details in user-facing text. Done — nothing else.
